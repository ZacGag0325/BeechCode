...
if (nzchar(configured_root)) {
  # An explicit root is authoritative. Do not reject it merely because another
  # copy of the workbook also exists elsewhere on the computer.
...
# Several discovery routes can point to the same workbook. Resolve those aliases
# before deciding that multiple distinct projects were found.
if (length(matches)) {
  matched_workbooks <- normalizePath(
    workbook_candidates[matches], winslash = "/", mustWork = TRUE
  )
  matches <- matches[!duplicated(matched_workbooks)]
}
...
if (length(matches) > 1L) {
  stop(
    "Found the workbook in more than one BeechCode project; refusing to choose silently:\n- ",
    paste(workbook_candidates[matches], collapse = "\n- "),
    "\nSet BEECHCODE_ROOT to select one project.", call. = FALSE
  )
}

project_root <- normalizePath(root_candidates[matches], winslash = "/", mustWork = TRUE)
input_path <- normalizePath(workbook_candidates[matches], winslash = "/", mustWork = TRUE)
...
valid_coordinate <- !is.na(lat_value) & lat_value >= -90 & lat_value <= 90 &
  !is.na(lon_value) & lon_value >= -180 & lon_value <= 180
