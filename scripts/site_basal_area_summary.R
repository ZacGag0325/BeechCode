...
is_beech <- function(x) {
  txt <- clean_text(x)
  str_detect(txt, "fagus|grandifolia|beech|hetre|hetre commun|hetre$|hetre")
}
...
species_patterns <- c("species", "specie", "taxon", "espece", "essence", "latin", "nom")
...
summarize_basal_area <- function(df, dataset_name, has_species = TRUE) {
  ...
  if (is.na(species_col) && has_species) {
    warning("No species/taxon column detected in ", dataset_name,
            ". Beech-specific summaries from this table will be NA.")
  }
