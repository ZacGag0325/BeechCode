# CELL 3 — create missing scripts/08_ibs_ibd.R
writeLines(c(
  '############################################################',
  ...
  'ids <- df_ids %>% dplyr::select(ind, Site, Latitude, Longitude) %>% dplyr::distinct(ind, .keep_all = TRUE)',
  'pair_rows <- list(); k <- 1L',
  'for (i in seq_len(length(inds)-1)) for (j in (i+1):length(inds)) {',
  '  id1 <- inds[i]; id2 <- inds[j]',
  '  r1 <- ids[ids$ind==id1, , drop=FALSE]; r2 <- ids[ids$ind==id2, , drop=FALSE]',
  '  if (nrow(r1)==0 || nrow(r2)==0) next',
  '  geo <- if (all(!is.na(c(r1$Latitude,r1$Longitude,r2$Latitude,r2$Longitude)))) haversine_km(r1$Latitude,r1$Longitude,r2$Latitude,r2$Longitude) else NA_real_',
  '  pair_rows[[k]] <- data.frame(ind_1=id1, ind_2=id2, site_1=as.character(r1$Site), site_2=as.character(r2$Site), genetic_distance=gen_dist[i,j], geographic_km=geo, stringsAsFactors=FALSE)',
  '  k <- k + 1L',
  '}',
