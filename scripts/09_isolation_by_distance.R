# CELL 4 — create missing scripts/09_isolation_by_distance.R
writeLines(c(
  '############################################################',
  ...
  'if (!file.exists(infile)) source(file.path("scripts","08_ibs_ibd.R"))',
  'd <- read.csv(infile, stringsAsFactors = FALSE) %>% dplyr::filter(!is.na(genetic_distance), !is.na(geographic_km))',
  ...
  'summary_tbl <- data.frame(n_pairs=nrow(d), slope=unname(coef(fit)["geographic_km"]), intercept=unname(coef(fit)["(Intercept)"]), r_squared=summary(fit)$r.squared, spearman_rho=unname(ct$estimate), spearman_p=ct$p.value)',
