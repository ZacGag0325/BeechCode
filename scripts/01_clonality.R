# filename: scripts/01_clone_correction.R
############################################################
...
if (length(missing) > 0) {
  stop("Missing required object files in ", OBJ_DIR, ": ", paste(missing, collapse = ", "))
}
...
if (!inherits(gi, "genind")) stop("gi.rds does not contain a genind object.")
...
miss_ind <- adegenet::info_table(gi, type = "missing", percent = TRUE)
if ("missing" %in% names(miss_ind)) {
  high_missing_n <- sum(miss_ind$missing > 35, na.rm = TRUE)
  if (high_missing_n > 0) {
...
map_mll <- tibble(ind = indNames(gi), mll_raw = mll_raw) %>%
  group_by(mll_raw) %>%
  summarise(cluster_size = n(), first_ind = min(ind), .groups = "drop") %>%
  arrange(desc(cluster_size), first_ind, mll_raw) %>%
  mutate(MLL = paste0("MLL", row_number()))
...
keep_tbl <- df_ids %>%
  ...
  mutate(.keep = row_number() == 1L) %>%
  ungroup()

kept_ids <- keep_tbl$ind[keep_tbl$.keep]
dropped_tbl <- keep_tbl %>%
  filter(!.keep) %>%
...
pop(gi_cc) <- as.factor(keep_tbl$Site[keep_tbl$.keep])
